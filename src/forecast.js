...
import { getApiForecast, getApiForecast } from "./js/api";

import { convertData, mini, convertInCelsius } from "./js/helper";

export async function initForForecast() {
  if (localStorage.getItem("nameCity")) {
    ...
    const data = await getApiForecast(city);
    filterTime(data);
  }
}
initForForecast();
...
export function filterTime(data) {
  ...
  function insertDataInDom(data) {
    for (let i = 0; i <= 4; i++) {
      const mainblock = document.querySelector(".container__forecast");
      ...
      nameP.classList.add("forecast__temp");
      const spanP = document.createElement("span");
      const dateP = document.createElement("p");
      dateP.classList.add("forecast__title");
